...
export default function LectureViewer({ courseId, lectureId, onNavigate }: LectureViewerProps) {
  ...
  const handleSummarize = async () => {
    ...
    setSummaryError("");
    try {
     const res = await aiApi.summarize({ course_id: cId, slide_id: lId });
      setSummaryData({
        summary: res.summary,
        ...
        {summaryData && !summaryLoading && (
          <div className="space-y-5">
            <div className="flex items-center gap-2">
              ...
              <div className="text-sm text-[#3A4A5E] leading-relaxed prose prose-sm max-w-none">
                <ReactMarkdown
                  components={{
                    p: ({ children }) => (
                    ...
                    h1: ({ children }) => (
                      <h1 className="text-lg font-bold text-[#0D1B2E] mb-2">
                        {children}
                      </h1>
                    ),
                    ...
                    blockquote: ({ children }) => (
                      <blockquote className="border-l-4 border-[#1C3D6E] pl-3 my-3 text-[#5A6A82]">
                        {children}
                      </blockquote>
                    ),
                  }}
