...
const adminPages: Page[] = ["admin", "admin-courses", "admin-lectures", "admin-users"];

function isAdminOnlyPage(page: Page) {
...
export default function App() {
  ...
  useEffect(() => {
    if (!user) return;
    setNav({ page: isAdmin ? "admin" : "dashboard" });
  }, [isAdmin, user?.id]);
  ...
  const fallbackPage: Page = isAdmin ? "admin" : "dashboard";
  const activePage = !isAdmin && isAdminOnlyPage(nav.page) ? fallbackPage : nav.page;

  const navigate = (page: Page, extra?: Record<string, string>) => {
    const nextPage = !isAdmin && isAdminOnlyPage(page) ? fallbackPage : page;
    setNav({ page: nextPage, ...extra });
    setSidebarOpen(false);
    window.scrollTo({ top: 0, behavior: "smooth" });
  };

  const fullHeight = activePage === "lecture" || activePage === "ai";
      ...
      <div className="flex-1 flex flex-col min-h-screen lg:ml-[240px]">
        ...
        <header className="lg:hidden flex items-center gap-3 bg-white border-b border-[#DEE5F0] px-4 h-14 flex-shrink-0 sticky top-0 z-20">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-bold text-[#0D1B2E]" style={{ fontFamily: "Instrument Sans, sans-serif" }}>
              AI University LMS
            </span>
          </div>
        ...
        <main className={`flex-1 ${fullHeight ? "overflow-hidden flex flex-col" : "overflow-y-auto"}`}>
          ...
          {activePage === "courses" && <Courses onNavigate={navigate} />}
          {activePage === "course-detail" && (
            <CourseDetail courseId={nav.courseId || ""} onNavigate={navigate} />
          )}
          {activePage === "lecture" && (
            <LectureViewer
              courseId={nav.courseId || ""}
              lectureId={nav.lectureId || ""}
              onNavigate={navigate}
            />
          )}
          {activePage === "ai" && (
            <AIAssistant
              initialCourseId={nav.courseId}
              initialLectureId={nav.lectureId}
              initialScope={nav.scope}
            />
          )}
          {activePage === "profile" && <Profile />}
          {isAdmin && activePage === "admin" && <AdminDashboard onNavigate={navigate} />}
          {isAdmin && activePage === "admin-courses" && <AdminCourses />}
          {isAdmin && activePage === "admin-lectures" && <AdminLectures />}
          {isAdmin && activePage === "admin-users" && <AdminUsers />}
        </main>
      </div>
    </div>
  );
